<template>
  <div class="min-h-screen py-20 px-4 sm:px-6 lg:px-8" ref="projectsContainer">
    <div class="max-w-7xl mx-auto">
      ...
      <section class="mb-24">
        <div class="flex items-center space-x-4 mb-10">
          <div
            class="w-12 h-12 bg-gradient-to-br from-primary-500 to-accent-500 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0"
          >
        ...
        <div class="grid sm:grid-cols-2 gap-8">
          ...
          >
            <div class="flex items-center justify-between mb-6">
              ...
              <div
                v-else
                class="flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-500 to-accent-500 text-white shadow-lg transition-transform duration-300 group-hover:scale-110"
                role="img"
            ...
            <p
              class="text-base text-slate-600 dark:text-slate-300 leading-relaxed font-light mb-6 flex-1"
            >
              {{ app.description }}
            </p>
      ...
      <section class="mb-24">
        <div class="flex items-center space-x-4 mb-10">
          <div
            class="w-12 h-12 bg-gradient-to-br from-accent-500 to-primary-500 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0"
          >
        ...
        <div class="space-y-10">
          ...
          >
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6 gap-4">
              <div class="flex-1">
                <div class="flex items-center flex-wrap gap-3 mb-2">
                  ...
                  <span
                    class="px-3 py-1 text-xs font-bold rounded-full bg-gradient-to-r from-accent-500 to-primary-500 text-white"
                  >
                    {{ project.year }}
                  </span>
                </div>
                <p class="text-base md:text-lg font-medium text-primary-600 dark:text-primary-400">
                  {{ project.tagline }}
                </p>
              </div>
              <div class="flex flex-wrap gap-3">
                ...
                >
                  <component
                    :is="link.text === 'npm' ? Package : Github"
            ...
            <p
              class="text-base sm:text-lg text-slate-600 dark:text-slate-300 leading-relaxed font-light mb-8"
            >
              {{ project.description }}
            </p>
      ...
      <section>
        <div class="flex items-center space-x-4 mb-10">
          <div
            class="w-12 h-12 bg-gradient-to-br from-primary-500 to-primary-600 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0"
          >
          ...
          <div>
            ...
            <p class="text-base sm:text-lg text-slate-600 dark:text-slate-400 font-light">
              Shipping production systems across enterprise teams since 2018
            </p>
          </div>
        </div>
        ...
        <div class="space-y-16">
          <div v-for="(project, index) in projects" :key="index" class="group">
            <!-- Project Card -->
            <div
              class="bg-white/10 dark:bg-slate-800/20 backdrop-blur-sm rounded-3xl p-8 md:p-12 border border-white/20 dark:border-slate-700/20 hover:bg-white/20 dark:hover:bg-slate-800/30 hover:scale-[1.02] transition-all duration-500 shadow-2xl"
            >
              <!-- Project Header -->
              <div class="flex flex-col lg:flex-row items-start justify-between mb-8">
                <div class="flex-1 mb-6 lg:mb-0">
                  <div class="flex items-center space-x-3 sm:space-x-4 mb-4">
                    <div
                      class="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-gradient-to-br from-primary-500 to-primary-600 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0"
                    >
                      <Briefcase class="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-white" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <h2
                        class="text-lg sm:text-xl md:text-4xl font-bold text-slate-900 dark:text-white leading-tight"
                      >
                        {{ project.title }}
                      </h2>
                    </div>
                  ...
                  <p
                    class="text-base sm:text-lg md:text-xl lg:text-2xl text-slate-600 dark:text-slate-300 leading-relaxed font-light"
                  >
                    {{ project.description }}
                  </p>
                </div>
              ...
              <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-4">
                  <h3
                    class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center"
                  >
                    <Code
                      class="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-primary-500 flex-shrink-0"
                    />
                    Technologies
                  </h3>
                  <div class="flex flex-wrap gap-3">
                ...
                <!-- Achievements -->
                <div
                  v-if="getProjectAchievements(project).length > 0"
                  class="space-y-4 md:col-span-2 lg:col-span-2"
                >
                  <h3
                    class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center"
                  >
                    <Award
                      class="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-accent-500 flex-shrink-0"
                    />
                    Key Achievements
                  </h3>
                  <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    >
                      <CheckCircle
                        class="w-5 h-5 sm:w-6 sm:h-6 text-green-500 mt-0.5 flex-shrink-0"
                      />
                      <span class="text-slate-700 dark:text-slate-300 leading-relaxed">{{
                        achievement
                      }}</span>
              ...
              <!-- Links -->
              <div
                v-if="getProjectLinks(project).length > 0"
                class="mt-8 pt-8 border-t border-white/10 dark:border-slate-700/20"
              >
                <h3
                  class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center mb-4"
                >
                  <ExternalLink
                    class="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-blue-500 flex-shrink-0"
                  />
                  Project Links
                </h3>
